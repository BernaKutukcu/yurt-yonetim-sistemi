...
@section('title', 'Oda Ekle')
@section('page-title', 'Oda Ekle')
        ...
        <form method="POST" action="{{ route('admin.rooms.store') }}">
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="text" name="room_number" value="{{ old('room_number') }}" placeholder="101" maxlength="3" oninput="this.value=this.value.replace(/[^0-9]/g,'')" required>
                </div>
                <div class="form-group">
                    <label>Blok</label>
                    <select name="block" required>
                        <option value="">— Blok Seç —</option>
                        <option value="A" {{ old('block') == 'A' ? 'selected' : '' }}>A</option>
                        <option value="B" {{ old('block') == 'B' ? 'selected' : '' }}>B</option>
                        <option value="C" {{ old('block') == 'C' ? 'selected' : '' }}>C</option>
                        <option value="D" {{ old('block') == 'D' ? 'selected' : '' }}>D</option>
                    </select>
                </div>
                <div class="form-group">
                    <label>Kat</label>
